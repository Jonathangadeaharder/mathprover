...
MAX_DEPTH = 4  # max recursive-decomposition depth before a hard leaf is blocked (→ research, step 4)
...
def _ctx(root: Path, node: D.Node) -> T.Ctx:
    ...
    return T.Ctx(project_root=root, node=node.id, proof_dir=af.parent, attempt_file=af,
...
def verify_only(root: Path, dag: D.DAG) -> None:
    ...
    for nid in proved:
        ...
        if ok:
            ...
            n.status, n.proof_path = "open", ""
            n.attempts.append({"ts": int(time.time()), "verdict": "gate_fail",
                               "feedback_head": msg.splitlines()[0][:160] if msg else ""})
            P.LOG.info("  %s: GATE FAIL -> open :: %s", nid, msg.splitlines()[0][:120] if msg else "")
        dag.save(dpath)
    print("\n=== verify-only result ===")
    print(dag.summary())
    if dag.goal:
...
def materialize_children(root: Path, dag: D.DAG, parent_id: str,
                         sublemmas: list[dict], parent_proof: str) -> list[str]:
    ...
    for sub in sublemmas:
        ...
        dag.nodes[cid] = D.Node(id=cid, statement=src, status="open", main_decl=(decls[-1] if decls else name),
                                provides=decls, folder=f"proofs/{cid}")
...
def assemble_parent(root: Path, dag: D.DAG, parent_id: str) -> tuple[bool, str]:
    ...
    if not pp.startswith("by"):
        pp = "by " + pp
    cand = (P._imports_of(parent.statement) + "\n\n" + "\n\n".join(bodies) + "\n\n"
            + f"{AG._node_statement(parent.statement)} := {pp}\n")
    return AG._gate(_ctx(root, parent), cand)


# ---------------- batched scheduler (needs models) ----------------
def schedule(root: Path, dag: D.DAG, *, max_hours: float, max_nodes: int, allow_research: bool,
             research_threads: int = 3, research_rounds: int = 4) -> None:
    logpath = P.init_log(root)
    dpath = _dag_path(root)
    deadline = (time.time() + max_hours * 3600.0) if max_hours > 0 else None
    P.init_progress(root, task=f"dag:{dag.goal or 'goal'}", mode="orchestrator",
                    budget=(f"{max_hours}h" if deadline else f"{max_nodes} nodes"))
    residency.free()
    closed = 0
    while closed < max_nodes:
        if deadline and time.time() > deadline:
            P.LOG.info("orchestrator: wall-clock deadline"); break
        if dag.goal and dag.proved(dag.goal):
            P.LOG.info("orchestrator: GOAL PROVED"); break
        frontier = dag.frontier()
        if not frontier:
            P.LOG.info("orchestrator: frontier empty"); break
        ...
        if leaves:
            ...
            for nid in leaves:
                ...
                if status == "FALSE":
                    n.status = "refuted"; dag.save(dpath); progressed = True; continue
                ok, msg = AG.assemble_and_gate(ctxs[nid], plans[nid], proved_leaves)
                n.attempts.append({"ts": int(time.time()), "model": M.PROVER_MODEL,
                ...
                if n.depth < MAX_DEPTH:
                    if not pl.get("direct", True) and pl.get("leaves") and pl.get("parent_proof"):
                        cand_split = {"sublemmas": [{"name": l.get("name", f"h{i}"), "statement": l.get("goal_spec", "")}
                                                    for i, l in enumerate(pl["leaves"])],
                    ...
                    if split is None:
                        ...
                        if ps.get("sublemmas") and P.verify_split(n.statement, ps, root):
                            split = ps
                if split:
                    kids = materialize_children(root, dag, nid, split["sublemmas"], split["parent_proof"])
                    progressed = True
                    P.LOG.info("  %s: DECOMPOSED -> %s", nid, kids)
                elif allow_research:
                    # ---- DEEP-RESEARCH (§5, local; no cloud) ----
                    n.status = "researching"; dag.save(dpath)
                    res = DR.deep_research(ctxs[nid], root, threads=research_threads,
                                           rounds=research_rounds, deadline=deadline)
                    ...
                    if res["status"] == "proved":
                        ok, _ = AG._gate(ctxs[nid], res["proof"])  # write node attempt.lean + final_verify
                        if ok:
                            af = _attempt_file(root, n)
                            axout = axiom_check(project_root=root, lean_file=af, theorem=n.main_decl or None)
                            n.axioms = _parse_axioms(axout.combined) or D.STD_AXIOMS
                        ...
                        kids = materialize_children(root, dag, nid, res["split"]["sublemmas"], res["split"]["parent_proof"])
                        progressed = True
                        P.LOG.info("  %s: deep-research DECOMPOSED -> %s", nid, kids)
                    else:
                        n.status = "blocked"
                        P.LOG.info("  %s: BLOCKED (deep-research exhausted; report %s)", nid, n.report)
                else:
                    n.status = "blocked"
        ...
        if not progressed:
            P.LOG.info("orchestrator: no progress this sweep -> stop"); break
    residency.free()
    print("\n=== orchestrator result ===")
    print(dag.summary())
    print(f"(log: {logpath})")
...
def main() -> None:
    ...
    ap.add_argument("--goal", default="", help="root node id or theorem name (seeds if no dag.json)")
    ap.add_argument("--verify-only", action="store_true", help="gate proved nodes; no models")
    ap.add_argument("--max-hours", type=float, default=0.0)
    ap.add_argument("--max-nodes", type=int, default=20, help="stop after this many newly-closed nodes")
    ap.add_argument("--allow-research", action="store_true", help="enable local deep-research on hard nodes")
    ap.add_argument("--research-threads", type=int, default=3, help="Heavy-mode angle-diverse threads")
    ...
    if a.verify_only:
        ...
        schedule(root, dag, max_hours=a.max_hours, max_nodes=a.max_nodes, allow_research=a.allow_research,
                 research_threads=a.research_threads, research_rounds=a.research_rounds)
