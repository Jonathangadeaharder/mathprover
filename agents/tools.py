...
from proof_architecture import (  # noqa: E402
    build_architecture,
    render_markdown,
    write_scratch_probe,
)
...
class Ctx:
    ...
    )  # gemma-selected premises (no nomic in hot path)
...
def _lexical_prefilter(goal_src: str, corpus: list[str], keep: int = 150) -> list[str]:
    ...
    gid = {
        t
        for t in re.findall(r"[A-Za-z_][A-Za-z0-9_']{2,}", goal_src.split(":=", 1)[0])
        if t not in kw
    }
    if not gid:
        return corpus[:keep]
    scored = sorted(
        corpus,
        key=lambda s: len(
            gid & {t for t in re.findall(r"[A-Za-z_][A-Za-z0-9_']{2,}", s) if t not in kw}
        ),
        reverse=True,
    )
    return scored[:keep]
...
def t_gather_context(ctx: Ctx, focus: str = "") -> str:
    ...
    out = M.chat_sync(
        ...
        [
            {
                "role": "system",
                "content": "You are a Lean 4 context librarian. You read large project context and emit a COMPACT, "
...
def t_run_lean(ctx: Ctx, snippet: str) -> str:
    r = compile_lean_file(
        project_root=ctx.project_root, lean_file=P._scratch(ctx.project_root, snippet)
    )
...
def t_check_decl(ctx: Ctx, name: str, imports: str = "import LBTCoupling") -> str:
    snip = f"{imports}\n#check @{name}\n"
    r = compile_lean_file(
        project_root=ctx.project_root, lean_file=P._scratch(ctx.project_root, snip)
    )
...
def t_open_private(ctx: Ctx, names: str, module: str) -> str:
    ...
    probe = write_scratch_probe(
        project_root=ctx.project_root,
        goal_file=ctx.attempt_file,
        names=namelist,
        open_private_from=module,
    )
    r = compile_lean_file(project_root=ctx.project_root, lean_file=probe)
    return (
        ("open-private OK: " + " ".join(namelist))
        if r.ok
...
def t_refute(ctx: Ctx, goal: str = "", rounds: int = 4) -> str:
    ...
    if cex:
        return (
            "COUNTEREXAMPLE FOUND — the statement is FALSE. Do not try to prove it; report FALSE."
        )
...
def t_final_verify(ctx: Ctx, theorem: str = "") -> str:
    ...
    ok, msg = final_verify_attempt(
        project_root=ctx.project_root, lean_file=ctx.attempt_file, theorem=theorem or None
    )
    ctx.verified_ok = ok
    ctx.last_verify_msg = msg
    return (
        "CLEAN (compiles, no placeholders, axioms ⊆ standard, statement preserved)"
        if ok
        else "FAIL:\n" + _clip(msg, 2000)
    )
...
def t_submit_aristotle(ctx: Ctx, node: str = "") -> str:
    ...
    return (
        "Cloud escalation is a separate, rate-limited tool. Run out-of-band:\n"
