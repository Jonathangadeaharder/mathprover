...
ANGLES = [
    (
        "direct",
        "Close the goal directly: find the single key lemma (search_premises/check_decl) and "
        "the tactic chain that applies it. Probe its exact type before proposing.",
    ),
    (
        "reduce",
        "Reduce to a known mathlib/project lemma: identify what standard result the goal is an "
        "instance of, verify it exists and its signature, then propose the reduction.",
    ),
    (
        "decompose",
        "Split into 2-4 GENERIC, easier sub-lemmas whose conjunction closes the goal; give a "
        "parent_proof that uses them by name. Prefer reusable Mathlib-shaped helpers.",
    ),
    (
        "cases",
        "Attack by case analysis / contradiction / induction: identify the right split variable "
...
def research_thread(ctx: T.Ctx, angle: tuple[str, str], deadline: float | None) -> dict:
    ...
    try:
        ...
        if pr.split:
            rep["split"] = {
                "sublemmas": [
                    {"name": s.name, "statement": s.statement} for s in pr.split.sublemmas
                ],
                "parent_proof": pr.split.parent_proof,
            }
        rep["confidence"] = 0.5 if rep["attempts"] or rep["split"] else 0.0
        P.LOG.info(
            "  research[%s]: proposed %d attempts%s",
            name,
            len(rep["attempts"]),
...
def synthesize(ctx: T.Ctx, reports: list[dict]) -> dict:
    ...
    try:
        ...
        pr: schemas.ProposeResult = result.output  # type: ignore[assignment]
        return {
            "attempts": [{"goal_spec": a.goal_spec, "sketch": a.sketch} for a in pr.attempts],
            "split": (
                {
                    "sublemmas": [
                        {"name": s.name, "statement": s.statement} for s in pr.split.sublemmas
...
def deep_research(
    ...
    *,
    ...
    for t in range(threads):
        ...
        P.prog(
            project_root,
            f"research thread {t} ({ANGLES[t % len(ANGLES)][0]})",
            model=M.PLANNER_MODEL,
        )
    ...
    if (
        isinstance(sp, dict)
        and sp.get("sublemmas")
        and sp.get("parent_proof")
...
def write_report(project_root: Path, node_id: str, reports: list[dict]) -> str:
    ...
    for r in reports:
        body += (
            f"## angle: {r['angle']}  (confidence {r['confidence']})\n"
