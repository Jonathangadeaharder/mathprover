...
def init_log(project_root: Path, run_id: str | None = None) -> Path:
    ...
    fh = logging.FileHandler(path, encoding="utf-8"); fh.setLevel(logging.DEBUG); fh.setFormatter(fmt)
...
ANSI_RE = re.compile(r"\x1b\[[0-9;]*m")  # strip ANSI so feedback is the raw text the model trained on
...
def init_progress(project_root: Path, *, task: str, mode: str, budget: str) -> None:
    ...
    global _PROG
    _PROG = {"task": task, "mode": mode, "budget": budget, "status": "running",
             "started": time.time(), "updated": time.time(), "round": 0,
             "phase": "", "model": "", "current": task, "result": "", "events": []}
...
def aristotle_user_prompt(proof_dir: Path, attempt_file: Path, project_root: Path,
                          prev: str | None, fb: str | None) -> str:
    ...
    base = build_aristotle_prompt(proof_dir=proof_dir, attempt_file=attempt_file, project_root=project_root)
    ...
    base = (f"The file `{rel}` currently contains:\n```lean\n{attempt_file.read_text(encoding='utf-8')}\n```\n\n"
            + base
...
def chat(model: str, messages: list[dict], *, max_tokens: int = 4096, temperature: float = 0.2) -> str:
...
def retrieve_premises(goal_src: str, project_root: Path, k: int = 12, pool: int = 600) -> list[str]:
    t0 = time.time()
    corpus = project_lemma_corpus(project_root)[:pool]  # cap the pool (was embedding ~4000 in one call)
    if not corpus:
    ...
    try:
        ...
        return []
    scored = sorted(zip(corpus, cvs), key=lambda cb: _cos(qv, cb[1]), reverse=True)
    LOG.info("retrieve_premises: %d/%d premises in %.1fs", k, len(corpus), time.time() - t0)
    return [c for c, _ in scored[:k]]


# ---------- roles ----------
def autoformalize(informal: str, project_root: Path, *, imports: str = "import Mathlib") -> str | None:
    """gemma: informal goal -> a compiling `sorry`-lemma. Compiler in loop (retry on type error)."""
    msgs = [{"role": "system", "content":
             "You are a Lean 4 autoformalizer. Output ONE ```lean block: necessary imports, then a "
    ...
    for _ in range(3):
        ...
        msgs += [{"role": "assistant", "content": out},
                 {"role": "user", "content": f"That did not typecheck:\n```\n{r.error_excerpt(1500)}\n```\nFix the STATEMENT."}]
    return None
...
def propose_split(goal_src: str, premises: list[str]) -> dict:
    ...
    try:
        ...
        return {"sublemmas": [{"name": s.name, "statement": s.statement} for s in sp.sublemmas],
...
def verify_split(goal_src: str, split: dict, project_root: Path) -> bool:
    ...
    LOG.debug("verify_split: %s%s", "OK (entails)" if r.ok else "FAIL",
...
def truth_probe(goal_src: str, project_root: Path) -> bool:
    ...
    for tac in ("decide", "norm_num", "simp_all", "omega"):
        ...
        if compile_lean_file(project_root=project_root, lean_file=_scratch(project_root, cand)).ok and not has_sorry(cand):
...
_GOAL_SKIP = {"theorem", "lemma", "by", "fun", "if", "then", "else", "let", "Nat", "Set", "Real",
              "Measure", "MeasureTheory", "ENNReal", "BigOperators", "Finset", "sorry", "open",
              "import", "Type", "Prop", "Population", "BitString"}


def definitions_for_goal(goal_src: str, project_root: Path, max_defs: int = 5, max_chars: int = 800) -> list[str]:
    ...
    for name in names:
        if len(out) >= max_defs:
            break
        pat = re.compile(rf"^(?:noncomputable\s+|private\s+|protected\s+)*(?:def|abbrev)\s+{re.escape(name)}\b", re.M)
        for f in files:
...
def pattern_hints(goal_src: str) -> list[str]:
    ...
    hits.append("If you cannot close this directly, introduce a small GENERIC helper lemma "
                "(state it, prove it first, then use it) — general facts are easier and reusable.")
    return hits


def _build_refine_prompt(goal_src: str, premises: list[str], defs: list[str], hints: list[str],
                         prev: str | None, fb: str | None) -> str:
...
def prove_leaf(goal_src: str, project_root: Path, sketch: str, premises: list[str],
               rounds: int | None = None, width: int = 4, deadline: float | None = None,
               prompt_override=None) -> str | None:
    ...
    sys_msg = ("You are OProver, a Lean 4 theorem prover. Given a goal to complete, relevant "
               ...
               "block with no `sorry`.")
    refs = ([f"Proof idea (strategy hint): {sketch}"] + list(premises)) if sketch else list(premises)
    defs = definitions_for_goal(goal_src, project_root)
    hints = pattern_hints(goal_src)
    LOG.info("  prove_leaf: %d in-scope definitions, %d premises, %d pattern hints",
             len(defs), len(premises), len(hints))
    k = max(1, width)
    ...
    while True:
        ...
        for i, (prev, fb) in enumerate(branches):
            LOG.info("  prove_leaf r%d b%d: querying %s (gen<=%d, T=%.1f)",
            ...
            if r.ok:
                ...
                name = (re.search(r"\b(?:theorem|lemma)\s+([A-Za-z0-9_']+)", cand) or [None, "leaf"])[1]
                out_f = saved / f"{name}.lean"
                out_f.write_text(cand, encoding="utf-8")
                LOG.info("  prove_leaf r%d b%d: VERIFIED (lake-clean, sorry-free, statement preserved) -> %s",
                         rnd, i, out_f)
...
def build_negation_goal(goal_src: str) -> str | None:
    ...
    neg = (f"theorem {name}_is_false :\n    ¬ (∀ {binders},\n    {concl}) := by\n  sorry\n"
           if binders else f"theorem {name}_is_false : ¬ ({concl}) := by\n  sorry\n")
...
def _scratch(project_root: Path, content: str) -> Path:
    d = project_root / ".mathprover" / "pipeline"; d.mkdir(parents=True, exist_ok=True)
    f = d / "scratch.lean"; f.write_text(content)
    return f
...
def solve(goal_src: str, project_root: Path, depth: int, max_depth: int, log) -> bool:
    ...
    if not verify_split(goal_src, split, project_root):
        log(f"{'  '*depth}  split REJECTED by compiler (does not entail parent); UNPROVED (local).")
        return False
    log(f"{'  '*depth}  split ENTAILS parent (compiler-checked): {[s['name'] for s in split['sublemmas']]}")
    return all(solve(s["statement"], project_root, depth + 1, max_depth, log) for s in split["sublemmas"])


def prove_node_longrun(root: Path, node: str, *, prompt_style: str, max_hours: float,
                       max_rounds: int, logpath: Path) -> None:
    ...
    if not attempt_file.exists():
        LOG.error("no attempt.lean for node %s", node); print("RESULT: NOT PROVED"); return
    ...
    proof = prove_leaf(src, root, "", premises,
                       rounds=(None if deadline else max_rounds), width=1,
                       deadline=deadline, prompt_override=override)
    if proof:
...
def main() -> None:
    ...
    ap.add_argument("--prompt", choices=["oprover", "aristotle"], default="oprover",
    ...
    ap.add_argument("--max-rounds", type=int, default=8, help="refinement-round cap when --max-hours=0")
    ...
    ap.add_argument("--refute-first", type=int, default=0, metavar="N",
    ...
    if a.node:
        prove_node_longrun(root, a.node, prompt_style=a.prompt, max_hours=a.max_hours,
                           max_rounds=a.max_rounds, logpath=logpath)
        return
    src = Path(a.goal_file).read_text() if a.goal_file else autoformalize(a.informal, root)
    if not src:
        LOG.error("autoformalize FAILED (no compiling statement)"); print("RESULT: NOT PROVED"); return
    LOG.info("goal:\n%s", src.strip())
    if a.refute:
