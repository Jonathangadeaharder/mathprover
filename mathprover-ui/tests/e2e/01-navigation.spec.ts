...
test.describe('Project picker → workspace navigation', () => {
  test('project picker renders when no project detected', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Open a Lean+LaTeX research project' })).toBeVisible();
  ...
  test('screenshot: project picker', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/01-project-picker.png', fullPage: true });
  });
  ...
  test('screenshot: workspace graph', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'tests/screenshots/02-workspace-graph.png', fullPage: true });
  });
});
