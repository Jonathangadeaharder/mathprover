...
test.describe("Graph view", () => {
  ...
  test("graph stage renders with aria label", async ({ page }) => {
    await expect(
      page.getByRole("application", { name: "Theorem dependency graph" }),
    ).toBeVisible();
  });
  ...
  test("graph renders theorem nodes", async ({ page }) => {
    ...
    await expect(
      stage.getByText("Constant-Ratio Runtime Theorem"),
    ).toBeVisible();
  ...
  test("graph legend renders all statuses", async ({ page }) => {
    ...
    await expect(
      legend.getByText("in_progress", { exact: true }),
    ).toBeVisible();
  ...
  test("screenshot: graph view", async ({ page }) => {
    ...
    await page.screenshot({
      path: "tests/screenshots/03-graph-view.png",
      fullPage: true,
    });
  });
});
