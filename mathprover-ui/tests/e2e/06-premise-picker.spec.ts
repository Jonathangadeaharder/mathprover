...
test.describe('Premise picker modal', () => {
  ...
  test('premise picker shows mathlib retrieval section', async ({ page }) => {
    ...
    await aside.getByRole('button', { name: 'Frontier' }).click();
    await page.getByRole('button', { name: 'Dispatch', exact: true }).first().click();
    await expect(page.getByText('Mathlib retrieval (semantic search)')).toBeVisible();
  });
  ...
  test('premise picker shows cancel and dispatch buttons', async ({ page }) => {
    ...
    await expect(dialog.getByRole('button', { name: /Dispatch/ })).toBeVisible();
  });
  ...
  test('premise picker shows real definitions from project data', async ({ page }) => {
    const aside = page.locator('aside.sidebar');
    await aside.getByRole('button', { name: 'Frontier' }).click();
    await page.getByRole('button', { name: 'Dispatch', exact: true }).first().click();
  ...
  test('screenshot: premise picker', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/12-premise-picker.png', fullPage: true });
  });
});
