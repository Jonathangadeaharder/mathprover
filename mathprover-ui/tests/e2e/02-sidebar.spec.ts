...
test.describe('Sidebar navigation + status counts', () => {
  ...
  test('clicking Definitions navigates to definitions view', async ({ page }) => {
    const aside = page.locator('aside.sidebar');
  ...
  test('clicking Failure shelf navigates to failures view', async ({ page }) => {
    const aside = page.locator('aside.sidebar');
    await aside.getByRole('button', { name: 'Failure shelf' }).click();
    await expect(page.getByRole('heading', { name: 'Failure shelf' })).toBeVisible();
  });

  test('clicking Foundations navigates to foundations view', async ({ page }) => {
    const aside = page.locator('aside.sidebar');
    await aside.getByRole('button', { name: 'Foundations' }).click();
    await expect(page.getByRole('heading', { name: 'Foundations' })).toBeVisible();
  });

  test('clicking Paper ⇄ Lean navigates to paper-lean view', async ({ page }) => {
    const aside = page.locator('aside.sidebar');
