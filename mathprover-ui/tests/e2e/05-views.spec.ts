...
test.describe('Tracks view', () => {
  ...
  test('screenshot: tracks view', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/05-tracks-view.png', fullPage: true });
  });
});
...
test.describe('Frontier view', () => {
  ...
  test('frontier shows dispatch button', async ({ page }) => {
    await expect(page.getByRole('button', { name: 'Dispatch', exact: true }).first()).toBeVisible();
  });
  ...
  test('frontier meter bars render with width', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      const width = await meterFills.nth(i).evaluate((el) => el.getBoundingClientRect().width);
  ...
  test('screenshot: frontier view', async ({ page }) => {
    await page.waitForTimeout(300);
    await page.screenshot({ path: 'tests/screenshots/06-frontier-view.png', fullPage: true });
  });
});
...
test.describe('Paper ⇄ Lean view', () => {
  ...
  test('paper-lean heading renders', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'Paper ⇄ Lean' })).toBeVisible();
  });
  ...
  test('screenshot: paper-lean view', async ({ page }) => {
    await page.waitForTimeout(300);
    await page.screenshot({ path: 'tests/screenshots/07-paper-lean-view.png', fullPage: true });
  });
});
...
test.describe('Agent runs view', () => {
  ...
  test('agent detail shows timeline and log tabs', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Streaming log' })).toBeVisible();
  });

  test('screenshot: agents view', async ({ page }) => {
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'tests/screenshots/08-agents-view.png', fullPage: true });
  });
});
...
test.describe('Failures view', () => {
  ...
  test('failures heading renders', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'Failure shelf' })).toBeVisible();
  });
  ...
  test('failures show view node button', async ({ page }) => {
    await expect(page.getByRole('button', { name: /view node/i }).first()).toBeVisible();
  });

  test('screenshot: failures view', async ({ page }) => {
    await page.waitForTimeout(300);
    await page.screenshot({ path: 'tests/screenshots/09-failures-view.png', fullPage: true });
  });
});
...
test.describe('Foundations view', () => {
  ...
  test('foundations render cards', async ({ page }) => {
    await expect(page.getByText('Stochastic Dominance Lemma')).toBeVisible();
    await expect(page.getByText('Markov Chain Hitting Time')).toBeVisible();
  });

  test('foundations show status pills', async ({ page }) => {
    await expect(page.locator('.found-card .status-pill').first()).toBeVisible();
  });
  ...
  test('screenshot: foundations view', async ({ page }) => {
    await page.waitForTimeout(300);
    await page.screenshot({ path: 'tests/screenshots/10-foundations-view.png', fullPage: true });
  });
});
...
test.describe('Definitions view', () => {
  ...
  test('screenshot: definitions view', async ({ page }) => {
    await page.waitForTimeout(300);
    await page.screenshot({ path: 'tests/screenshots/11-definitions-view.png', fullPage: true });
  });
});
