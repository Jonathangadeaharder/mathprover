...
test.describe("Node detail panel", () => {
  ...
  test("clicking node shows detail panel with heading", async ({ page }) => {
    const stage = page.getByRole("application", {
      name: "Theorem dependency graph",
    });
    await stage.getByText("Constant-Ratio Runtime Theorem").click();
    await expect(
      page.getByRole("heading", { name: "Constant-Ratio Runtime Theorem" }),
    ).toBeVisible();
  });
  ...
  test("detail panel shows paper statement", async ({ page }) => {
    ...
    await expect(
      overlay.getByText(
        "Every stochastically CRN admits a constant-ratio runtime bound.",
      ),
    ).toBeVisible();
  });
  ...
  test("detail panel shows tabs", async ({ page }) => {
    ...
    const overlay = page.locator(".detail-overlay");
    await expect(
      overlay.getByRole("button", { name: "Overview" }),
    ).toBeVisible();
    await expect(
      overlay.getByRole("button", { name: "Paper source" }),
    ).toBeVisible();
    await expect(
      overlay.getByRole("button", { name: "Lean code" }),
    ).toBeVisible();
    await expect(
      overlay.getByRole("button", { name: "Mapping" }),
    ).toBeVisible();
    await expect(
      overlay.getByRole("button", { name: "Attempts" }),
    ).toBeVisible();
    await expect(
      overlay.getByRole("button", { name: "Subgoals" }),
    ).toBeVisible();
  });
  ...
  test("detail panel close button works", async ({ page }) => {
    ...
    await stage.getByText("Kernel Founder Mass Lemma").click();
    await expect(
      page.getByRole("heading", { name: "Kernel Founder Mass Lemma" }),
    ).toBeVisible();
  ...
  test("Meter component renders with width (triple-style bug fix)", async ({
    page,
  }) => {
    const stage = page.getByRole("application", {
      name: "Theorem dependency graph",
    ...
    const meterFill = page
      .locator(".detail-overlay .meter-inline-fill")
      .first();
  ...
  test("attempts tab shows attempt log", async ({ page }) => {
    ...
    await expect(
      overlay.getByText("direct_search", { exact: true }),
    ).toBeVisible();
  });
  ...
  test("screenshot: node detail", async ({ page }) => {
    ...
    await page.screenshot({
      path: "tests/screenshots/04-node-detail.png",
      fullPage: true,
    });
  });
});
