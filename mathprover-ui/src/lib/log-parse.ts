...
const PATTERNS: { re: RegExp; lvl: string; title?: string }[] = [
  ...
  {
    re: /^extract: no lean4 block/,
    lvl: "warn",
    title: "No Lean block in output",
  },
  {
    re: /^extract: candidate still contains sorry/,
    lvl: "warn",
    title: "Proof still has sorry",
  },
...
export function parseLogText(raw: string, startedAt?: string): LogLine[] {
  ...
  for (const line of raw.split("\n")) {
    ...
    const lvl =
      matched?.lvl ?? (trimmed.startsWith("warning:") ? "warn" : "info");
