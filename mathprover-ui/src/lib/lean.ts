...
export const TERM_LOOKUP: TermLookup = new Proxy({} as TermLookup, {
  get(_t, p) {
    return Reflect.get(project.data.terms, p);
  },
  has(_t, p) {
    return Reflect.has(project.data.terms, p);
  },
  ownKeys() {
    return Reflect.ownKeys(project.data.terms);
  },
...
export function highlightLean(src: string): string {
  ...
  s = s.replace(
    ...
    (m) => {
      ...
      if (lookup[m])
        return `<span class="kw lean-term" data-term="${m}">${m}</span>`;
      return `<span class="kw">${m}</span>`;
    },
  );
  s = s.replace(/\b([A-Z][A-Za-z0-9_]*)\b/g, (_m, name: string) => {
    if (lookup[name])
      return `<span class="tp lean-term" data-term="${name}">${name}</span>`;
  ...
  for (const name of Object.keys(lookup)) {
    if (/^[a-z]/.test(name) && !KEYWORDS.has(name)) {
      ...
      s = s.replace(
        re,
        `<span class="fn lean-term" data-term="${name}">$1</span>`,
      );
    }
  }
